...
extern UINT WatchdogPid;	/* ASUS EXT */
...
void char_to_ascii(char *output, char *input)	/* ASUS EXT */
{
	int i;
	char tmp[10];
	char *ptr;
	...
	IN  UCHAR			CurOpMode)
{
	...
	char tmpstr[128];				/* ASUS EXT */
	UCHAR tmpstr2[32], tempRandomByte = 0, idx = 0;	/* ASUS EXT */
...
/* ASUS EXT */
				if (apidx == 0)
				{
					...
					nvram_set("wl0_ssid", pAd->ApCfg.MBSSID[apidx].Ssid);
					if (nvram_match("wl1_wsc_config_state", "0"))
					{
#if 1
						nvram_set("wl1_ssid", pAd->ApCfg.MBSSID[apidx].Ssid);
#else
						...
						nvram_set("wl1_ssid", tmpstr2);
#endif
					}

					memset(tmpstr, 0x0, 128);
					char_to_ascii(tmpstr, pAd->ApCfg.MBSSID[apidx].Ssid);
					nvram_set("wl0_ssid2", tmpstr);
					if (nvram_match("wl1_wsc_config_state", "0"))
					{
#if 1
						nvram_set("wl1_ssid2", tmpstr);
#else
						memset(tmpstr, 0x0, 128);
...
/* ASUS EXT */
				if (apidx == 0)
				{
					...
					nvram_set("wl0_wpa_psk", tmpstr);
					if (nvram_match("wl1_wsc_config_state", "0"))
					nvram_set("wl1_wpa_psk", tmpstr);
				}
/* ASUS EXT */
			}
		}
...
/* ASUS EXT */
				if (apidx == 0)
				{
					...
					printk("Current WPA-PSK=%s\n", tmpstr);
					nvram_set("wl0_wpa_psk", tmpstr);
					if (nvram_match("wl1_config_state", "0"))
					nvram_set("wl1_wpa_psk", tmpstr);
				}
/* ASUS EXT */
			}
...
#else				/* ASUS EXT */
				UCHAR tmpstr[32], tempRandomByte = 0, idx = 0;
				memset(tmpstr, 0x0, 32);
				sprintf((PSTRING) tmpstr, "%s", "ASUS_");
				for(idx = 0; idx < 5; idx++)
				{
					tempRandomByte = RandomByte(pAd)%94+33; // Generate printable ASCII (decimal 33 to 126)
					if (    tempRandomByte == 34 ||	// "
						...
						tempRandomByte = 48;	// "0"
					sprintf((PSTRING) tmpstr, "%s%c", tmpstr, tempRandomByte);
				}
				NdisMoveMemory(pWscControl->WscDefaultSsid.Ssid, tmpstr, 10);
				NdisMoveMemory(pCredential->SSID.Ssid, tmpstr, 10);
				pWscControl->WscDefaultSsid.SsidLength = 10;
				pCredential->SSID.SsidLength = 10;
#endif				/* ASUS EXT */
			}
			else
			{
...
/* ASUS EXT */
									...
									{
										nvram_set("wl0_auth_mode_x", "wpa");

										if (nvram_match("wl1_wsc_config_state", "0"))
										{
											nvram_set("wl1_auth_mode_x", "wpa");
										}
									}
									...
									{
										nvram_set("wl0_auth_mode_x", "wpa2");

										if (nvram_match("wl1_wsc_config_state", "0"))
										{
											nvram_set("wl1_auth_mode_x", "wpa2");
										}
									}
...
/* ASUS EXT */
								printk("Current DefaultKeyID=%d\n", pAd->ApCfg.MBSSID[index].DefaultKeyId+1);

								memset(tmpstr, 0x0, 128);
								sprintf(tmpstr, "%d", pAd->ApCfg.MBSSID[index].DefaultKeyId+1);
   						               	nvram_set("wl0_key", tmpstr);
   						               	if (nvram_match("wl1_wsc_config_state", "0"))
   						               	nvram_set("wl1_key", tmpstr);
/* ASUS EXT */
							}
							else
								snprintf(pTempStr, 512, "%s;%d", pTempStr, pAd->ApCfg.MBSSID[index].DefaultKeyId+1);
...
/* ASUS EXT */
						if (apidx == 0)
						{
							...
							{
								nvram_set("wl0_wep_x", "2");
								if (nvram_match("wl1_wsc_config_state", "0"))
								nvram_set("wl1_wep_x", "2");
							}
						}
/* ASUS EXT */
					}
							...
							if (apidx == 0)
							{
								...
								if ((pCredentail->KeyLength == 5) || (pCredentail->KeyLength == 13))
								{
									...
									if (flag_hex)
									{
										...
										nvram_set("wl0_key_type", "0");
										nvram_set("wl0_key1", tmpstr);
										nvram_set("wl0_key2", tmpstr);
										nvram_set("wl0_key3", tmpstr);
										nvram_set("wl0_key4", tmpstr);

										if (nvram_match("wl1_wsc_config_state", "0"))
										{
											nvram_set("wl1_key_type", "0");
											nvram_set("wl1_key1", tmpstr);
											nvram_set("wl1_key2", tmpstr);
											nvram_set("wl1_key3", tmpstr);
											nvram_set("wl1_key4", tmpstr);
										}
									...
									else
									{
										...
										nvram_set("wl0_key1", pCredentail->Key);
										nvram_set("wl0_key2", pCredentail->Key);
										nvram_set("wl0_key3", pCredentail->Key);
										nvram_set("wl0_key4", pCredentail->Key);

										if (nvram_match("wl1_wsc_config_state", "0"))
										{
											nvram_set("wl1_key_type", "1");
											nvram_set("wl1_key1", pCredentail->Key);
											nvram_set("wl1_key2", pCredentail->Key);
											nvram_set("wl1_key3", pCredentail->Key);
											nvram_set("wl1_key4", pCredentail->Key);
										}
...
/* ASUS EXT */
		...
		if (nvram_match("wl1_wsc_config_state", "0"))
		nvram_set("wl1_wsc_config_state", "2");
		nvram_set("w_Setting", "1");
		nvram_commit();
		if (WatchdogPid)
			kill_proc(WatchdogPid, SIGTTIN, 1);
		else
			printk("WatchdogPid is 0!\n");
#endif
/* ASUS EXT */
	}
