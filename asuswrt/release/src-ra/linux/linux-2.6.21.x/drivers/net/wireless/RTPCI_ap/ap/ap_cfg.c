	...
	IN      PUCHAR			arg)
{
	...
	if (temp > 0)
	{
		...
		printk("invalid watchdog pid value: %d\n", temp);

	if (nvram_match("debug_mgmtring", "1"))
		debug_mgmtring = 1;
	else
		debug_mgmtring = 0;
...
#define	MAX_BUFFER_SIZE	4096	// ASUS EXT
CHAR msg[MAX_BUFFER_SIZE];	// ASUS EXT

VOID	RTMPIoctlGetSTAINFO(	// ASUS EXT
	IN	PRTMP_ADAPTER	pAd, 
	IN	RTMP_IOCTL_INPUT_STRUCT *wrq)
{
	INT i;//, QueIdx=0;
...
VOID	RTMPIoctlGetSTAT(	// ASUS EXT
	IN	PRTMP_ADAPTER	pAd, 
	IN	RTMP_IOCTL_INPUT_STRUCT *wrq)
{
	INT Status=0;
	...
	// Sanity check for calculation of sucessful count
	sprintf(msg+strlen(msg), "TransmitCountFromOS = %d\n", pAd->WlanCounters.TransmitCountFrmOs.u.LowPart);
...
#endif
	sprintf(msg+strlen(msg), "TransmittedFrameCount = %d\n", pAd->WlanCounters.TransmittedFragmentCount.u.LowPart);
	sprintf(msg+strlen(msg), "WEPUndecryptableCount = %d\n", pAd->WlanCounters.WEPUndecryptableCount.u.LowPart);
...
#if MAX_AGG_CNT>8
	for (ii=0; ii<(MAX_AGG_CNT-8)/2; ii++) {
		RTMP_IO_READ32(pAd, aggReg[ii], &TxAggCntN.word);
		aggCnt[2*ii + 8] = TxAggCntN.field.AggSizeLowCount;
		aggCnt[2*ii + 9] = TxAggCntN.field.AggSizeHighCount;
	}
#endif

	totalCount = TxAggCnt.field.NonAggTxCount + TxAggCnt.field.AggTxCount;
	if (totalCount > 0)
		for (ii=0; ii<MAX_AGG_CNT; ii++) {
			sprintf(msg+strlen(msg), "\t%d MPDU=%d (%d%%)\n", ii+1, aggCnt[ii], aggCnt[ii]*100/totalCount);
		}
	sprintf(msg+strlen(msg), "====================\n");
...
VOID	RTMPIoctlGetRSSI(	// ASUS EXT
	IN PRTMP_ADAPTER pAd, 
	IN RTMP_IOCTL_INPUT_STRUCT *wrq)
{
	INT	Status;
	...
	kfree(msg);
	DBGPRINT(RT_DEBUG_TRACE, ("<==RTMPIoctlGetRSSI\n"));
}

VOID	RTMPIoctlGetRadioStatus(
	IN PRTMP_ADAPTER pAd,
	IN RTMP_IOCTL_INPUT_STRUCT *wrq)
{
	INT     Status;
	...
	RadioStatus = !RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_RADIO_OFF);

	wrq->u.data.length = sizeof(UINT);
	Status = copy_to_user(wrq->u.data.pointer, &RadioStatus, wrq->u.data.length);
}
	...
	IN RTMP_IOCTL_INPUT_STRUCT *wrq)
{
	...
	os_alloc_mem(NULL, (UCHAR **)&ptr0, 256);
	if(ptr0 == NULL)
	{
		ptr1 = "";
	}
	else
	{
		...
		{
			if(p == ptr1)
				p += sprintf(p,  "%d", pAd->ChannelList[i].Channel);
			else
				p += sprintf(p, ",%d", pAd->ChannelList[i].Channel);
		}
	}
	wrq->u.data.length = strlen(ptr1) + 1;	//including '\0'
	copy_to_user(wrq->u.data.pointer, ptr1, wrq->u.data.length);
	if(ptr0 != NULL)
		os_free_mem(NULL, ptr0);
	DBGPRINT(RT_DEBUG_TRACE, ("<==RTMPIoctlGetChannelList\n"));
}
