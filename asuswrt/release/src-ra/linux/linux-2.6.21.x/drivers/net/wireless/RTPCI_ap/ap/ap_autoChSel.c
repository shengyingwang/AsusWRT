...
#else
		printk("%s Fail to alloc memory for pAd->pBssInfoTab", __FUNCTION__);
#endif
	}
...
#else
		printk("%s Fail to alloc memory for pAd->pChannelInfo", __FUNCTION__);
#endif
	}
...
#else
           printk("%s(): Channel Selection not checked because there are %u associated clients.\n",
                   __FUNCTION__, pAd->MacTab.Size);
#endif
           return;
       }
