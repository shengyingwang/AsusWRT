...
#define IS_MULTICAST_LINKLOCAL_ADDR(_addr) (((_addr) & 0xffffff00) == 0xe0000000)
#define IS_MULTICAST_UPNP_SSDP_ADDR(_addr) ((_addr) == 0xeffffffa)
#define IS_MULTICAST_IPV6_RESERVED_ADDR(_addr) \
	((_addr)->ipv6_addr[0] == 0xff && (_addr)->ipv6_addr[1] == 0x00)
#define IS_MULTICAST_IPV6_LINKLOCAL_ADDR(_addr) \
	((_addr)->ipv6_addr[0] == 0xff && (_addr)->ipv6_addr[1] == 0x02)

static const RT_IPV6_ADDR IPv6MulticastFilterReserved[] =
{
	{.ipv6_addr16 = {0xff00,0,0,0,0,0,0,0x0000}}, /* Reserved ff0x:: */
	{.ipv6_addr16 = {0xff00,0,0,0,0,0,0,0x000c}}, /* SSDP ff0x::c */
	{.ipv6_addr16 = {0xff00,0,0,0,0,0,0,0x00fb}}, /* mDNSv6 ff0x::fb */
	{.ipv6_addr16 = {0xff00,0,0,0,0,0,0,0x0130}}, /* UPnP ff0x::130 */
};
#define IPV6_MULTICAST_FILTER_RESERVED_SIZE  \
	(sizeof(IPv6MulticastFilterReserved) / sizeof(RT_IPV6_ADDR))
	...
	IN PUCHAR pIpHeader)
{
	...
	if((UCHAR)*(pIpHeader + 11) == IGMP_PROTOCOL_DESCRIPTOR)
		return TRUE;
	...
	DstIpAddr = ntohl(*((UINT32*)(pIpHeader + 18)));

	if(IS_MULTICAST_LINKLOCAL_ADDR(DstIpAddr) ||
	   IS_MULTICAST_UPNP_SSDP_ADDR(DstIpAddr))
		return TRUE;
	...
	OUT PMULTICAST_FILTER_TABLE_ENTRY *ppGroupEntry)
{
	if(IS_MULTICAST_MAC_ADDR(pSrcBufVA))
	{
		...
		if(ntohs(*((UINT16 *)(pIpHeader))) == ETH_P_IPV6)
			...
			IgmpMldPkt = IPv4MulticastFilterExcluded(pSrcBufVA, pIpHeader);
	...
	IN PUCHAR pIpHeader)
{
	...
	if(result != TRUE)
	{
		/* Packets with destination IPv6 address in the range ff00::/16 (reserved),
		 * ff02::/16 (link local) must be forwarded on all ports. Similarly specific
		 * protocol traffic such as UPnP, SSDP and mDNSv6 must be forwarded on to all
		 * the ports.
		 */

		if(IS_MULTICAST_IPV6_RESERVED_ADDR(pIpv6DstAddr) ||
		   IS_MULTICAST_IPV6_LINKLOCAL_ADDR(pIpv6DstAddr))
			result = TRUE;
		else
		if((pIpv6DstAddr->ipv6_addr[0]) == 0xff &&
		   (pIpv6DstAddr->ipv6_addr[1] & 0xf0) == 0x00)
		for (idx = 0; idx < IPV6_MULTICAST_FILTER_RESERVED_SIZE; idx++)
		{
			if(NdisEqualMemory((UCHAR *)(&(pIpv6DstAddr->ipv6_addr[2])),
					   (UCHAR *)(&(IPv6MulticastFilterReserved[idx].ipv6_addr[2])),
					   IPV6_ADDR_LEN - 2))
			{
				result = TRUE;
				break;
