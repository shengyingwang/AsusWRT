...
static const struct name_id_map {
	const char *name;
	uint64_t id;
} mfs[] = {
	{ "hc3", LZMA_MF_HC3 },
...
static int xz_options(char *argv[], int argc)
{
	...
	arg = (argc < 2) ? -1 : strtoul(argv[1], &end, 10);
	...
	if(strcmp(argv[0], "-Xbcj") == 0) {
	...
	} else if (strcmp(argv[0], "-Xnice") == 0) {
		if (!end || *end || arg < 2 || arg > 273)
			goto failed;
		nice = arg;
		...
		if(argc < 2)
			goto failed;
		for (i = 0; mfs[i].name; i++) {
			if (strcmp(argv[1], mfs[i].name) == 0) {
				mf = mfs[i].id;
				break;
			}
		}
...
void xz_usage()
{
	...
	fprintf(stderr,
		"\t  -Xnice <N>: set nice length of a match [2-273], default: %d)\n"
		"\t  -Xlc <N>:   set number of literal context bits - [0, 4], default: %d\n"
		"\t  -Xlp <N>:   set number of literal pos bits - [0, 4], default: %d\n"
		"\t  -Xpb <N>:   set number of pos bits - [0, 4], default: %d\n"
		"\t  -Xmf <id>:  set match finder - [hc3, hc4, bt2, bt3, bt4], default: %s\n",
		nice, lc, lp, pb,
		(mf == LZMA_MF_HC3) ? "hc3" :
		(mf == LZMA_MF_HC4) ? "hc4" :
		(mf == LZMA_MF_BT2) ? "bt2" :
